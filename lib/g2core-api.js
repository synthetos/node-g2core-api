 ...
 */
class G2Error {
  constructor(subname, message, data) {
    Error.call(this);
...
class G2coreAPI extends EventEmitter {
  ...
  constructor() {
    ...
    this.seenConnectionBanner = false;
  ...
  _handleLine(buffer) {
    ...
    if (part[0] === '{' /* make the IDE happy: } */) {
      ...
      if (jsObject.hasOwnProperty('r')) {
        ...
        this.emit('response', jsObject.r, footer);

        jsObject = jsObject.r;
      }

      if (jsObject.hasOwnProperty('er')) {
        this.emit('errorReport', jsObject.er);
      } else if (jsObject.hasOwnProperty('sr')) {
        this.emit('statusChanged', jsObject.sr);
      } else if (jsObject.hasOwnProperty('gc')) {
        this.emit('gcodeReceived', jsObject.gc);
      }
    }
  }; // _handleLine;
  ...
  open(path, options) {
    ...
    const parser = this.serialPortControl.pipe(new Readline({ delimiter: '\n' }))
    parser.on('data', this._handleLine.bind(this))
    ...
    this.serialPortControl.once('close', (err) => {
      ...
      if (this.serialPortControl !== null) {
        parser.removeListener('data', _onControlData);
  ...
  _openSecondChannel(doSetup) {
    if (this.dataPortPath) {
      this.serialPortData = new SerialPort(this.dataPortPath, this._baseOptions);
  ...
  _completeOpen(doSetup) {
    ...
    // this.serialPortControl.resume();

    // Prepare the event listeners
    let _onResponse = (r) => {
      // console.error('r: '+util.inspect(r));
      if (!this.seenConnectionBanner) {
        this.emit('connected', r);
        this.seenConnectionBanner = true;
        this._completeConnection(doSetup);
        return;
      }

      if (this.ignoredResponses > 0) {
    ...
    let _onStatusChanged = (sr) => {
      ...
      // 2	machine is in alarm, shutdown, or panic state (shut down)
      } else if (sr.stat == 2 || sr.stat == 12 || sr.stat == 13) {
 ...
 */
 _completeConnection(doSetup) {
  ...
  /**
   * set - set the value of a parameter on the machine (via JSON)
   ...
   */
  async openFirst(failIfMore = false, options = {}) {
  ...
  async list() {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      //   serialNumber: '0000-084a-29c6-5ad6',
      //   pnpId: undefined,
      //   locationId: '14300000',
      //   vendorId: '1d50',
      //   productId: '606d'
      ...
      if (item.manufacturer == 'FTDI') {
        g2s.push({ path: item.path });
      } else if (item.manufacturer == 'Synthetos') {
        if (g2s.length > 0 &&
          (g2s[g2s.length - 1].serialNumber = item.serialNumber)
        ) {
          g2s[g2s.length - 1].dataPortPath = item.path;
        } else {
          g2s.push({ path: item.path, serialNumber: item.serialNumber });
        }
